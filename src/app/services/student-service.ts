...
export class StudentService {
  ...
  getAll(page: number = 0): Observable<ResponseMessage<PageResponseDTO<Student>>> {
    return this.http
      .get<ResponseMessage<PageResponseDTO<Student>>>(`${this.apiUrl}?page=${page}`)
      .pipe(catchError(this.handleError));
  }

  create(student: CreateStudent): Observable<ResponseMessage<Student>> {
    return this.http
      .post<ResponseMessage<Student>>(this.apiUrl, student)
      .pipe(catchError(this.handleError));
  }
